defmodule NookBook.ACNH.Cache do
  ...
  defmodule Private do
    ...
    alias NookBook.Data.GenericCache
    ...
    def list_from_cache(namespace, function) do
      ...
      |> case do
        [] ->
          ...
          |> Enum.map(&GenericCache.set({namespace, &1["id"]}, &1))

        list ->
          list
      end
      |> Enum.sort_by(fn r -> String.capitalize(r["name"]["name-USen"]) end)
    end
    ...
    def record_from_cache(namespace, function, id) do
      ...
      |> case do
        nil ->
          ...
          GenericCache.set({namespace, id}, record)
